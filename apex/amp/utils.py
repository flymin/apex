...
def cached_cast(cast_fn, x, cache):
    ...
    if x in cache:
        ...
        if x.requires_grad and cached_x.requires_grad:
            ...
            if next_functions_available and \
                cached_x.grad_fn.next_functions[1][0].variable is not x:
        ...
        if torch.is_grad_enabled() and x.requires_grad != cached_x.requires_grad:
            del cache[x]
        elif x.requires_grad and cached_x.requires_grad and \
            not next_functions_available:
            del cache[x]
        else:
            return cached_x
